...
locals {
  ...
  instance_type_families = ["ecs.f1"]
}


data "alicloud_instance_types" "this" {
  instance_type_family = var.instance_type_family != "" && contains(local.instance_type_families, var.instance_type_family) ? var.instance_type_family : "ecs.f1"
...
module "ecs-instance" {
  ...
  instance_name     = "TF-compute-optimized-type-with-fpga"
  use_num_suffix    = true
  tags              = var.tags
