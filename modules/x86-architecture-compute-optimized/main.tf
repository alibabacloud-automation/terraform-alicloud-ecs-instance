...
locals {
  ...
  instance_type_families = ["ecs.c6", "ecs.c5", "ecs.ic5"]
}


data "alicloud_instance_types" "this" {
  instance_type_family = var.instance_type_family != "" && contains(local.instance_type_families, var.instance_type_family) ? var.instance_type_family : "ecs.c6"
...
module "ecs-instance" {
  ...
  instance_name     = "TF-x86-architecture-compute-optimized"
  use_num_suffix    = true
  tags              = var.tags
