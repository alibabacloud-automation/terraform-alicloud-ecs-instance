...
locals {
  ...
  instance_type_families = ["ecs.hfg6", "ecs.hfg5", "ecs.hfc5"]
}


data "alicloud_instance_types" "this" {
  instance_type_family = var.instance_type_family != "" && contains(local.instance_type_families, var.instance_type_family) ? var.instance_type_family : "ecs.hfg6"
...
module "ecs-instance" {
  ...
  instance_name     = "TF-x86-architecture-high-clock-speed"
  use_num_suffix    = true
  tags              = var.tags
