...
variable "tags" {
  ...
  default = {
    Created = "Terraform"
    Source  = "alibaba/ecs-instance/alicloud//modules/compute-optimized-type-with-gpu"
  }
}
...
variable "spot_strategy" {
  description = "The spot strategy of a Pay-As-You-Go instance, and it takes effect only when parameter 'instance_charge_type' is 'PostPaid'. Value range: 'NoSpot' (regular Pay-As-You-Go instance), 'SpotWithPriceLimit' (price threshold for a spot instance), 'SpotAsPriceGo' (price based on the highest Pay-As-You-Go instance)."
  type        = string
  default     = "NoSpot"
}

variable "spot_price_limit" {
  description = "The hourly price threshold of an instance, and it takes effect only when parameter 'spot_strategy' is 'SpotWithPriceLimit'. Maximum of three decimal places allowed."
  type        = number
  default     = 0
}