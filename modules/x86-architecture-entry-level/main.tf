...
locals {
  ...
  instance_type_families = ["ecs.t6", "ecs.t5"]
}


data "alicloud_instance_types" "this" {
  instance_type_family = var.instance_type_family != "" && contains(local.instance_type_families, var.instance_type_family) ? var.instance_type_family : "ecs.t6"
...
module "ecs-instance" {
  ...
  instance_name     = "TF-x86-architecture-entry-level"
  use_num_suffix    = true
  tags              = var.tags
