locals {
  ...
  instance_type_families = ["ecs.d1ne", "ecs.d1"]
}


data "alicloud_instance_types" "this" {
  instance_type_family = var.instance_type_family != "" && contains(local.instance_type_families, var.instance_type_family) ? var.instance_type_family : "ecs.d1ne"
...
module "ecs-instance" {
  ...
  instance_name     = "TF-x86-architecture-big-data"
  use_num_suffix    = true
  tags              = var.tags
