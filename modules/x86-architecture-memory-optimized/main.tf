...
data "alicloud_instance_types" "this" {
  instance_type_family = var.instance_type_family != "" && contains(local.instance_type_families, var.instance_type_family) ? var.instance_type_family : "ecs.r6"
...
module "ecs-instance" {
  ...
  instance_name     = "TF-x86-architecture-memory-optimized"
  use_num_suffix    = true
  tags              = var.tags
